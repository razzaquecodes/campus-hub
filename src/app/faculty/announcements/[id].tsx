...
import { ArrowLeft, Send, Paperclip, Users, Pin, Info, Save } from 'lucide-react-native';
...
export default function FacultyEditNotice() {
  ...
  const { activeNotices, editNotice } = useFacultyStore();
  ...
  useEffect(() => {
    ...
    if (notice) {
      ...
      setIsAll(notice.target.isAll);
      if (notice.target.branch) setBranch(notice.target.branch);
      if (notice.target.year) setYear(notice.target.year);
      else if (notice.target.semester) setYear(String(Math.ceil(Number.parseInt(notice.target.semester, 10) / 2)));
      if (notice.target.section) setSection(notice.target.section);
    }
  }, [id, activeNotices]);
    ...
    <View style={[ss.root, { backgroundColor: theme.colors.void }]}>
      ...
      <Animated.View entering={FadeInDown.duration(400)} style={[ss.header, { paddingTop: insets.top + Spacing.sm }]}>
        ...
        <View style={{ flex: 1, marginLeft: 16 }}>
          <Text style={[Typography.display.small, { color: theme.colors.textPrimary, letterSpacing: -0.5 }]}>Edit Broadcast</Text>
        </View>
      </Animated.View>
      ...
      <Animated.View entering={FadeInUp.duration(500).delay(400)} style={[ss.footerArea, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, paddingBottom: insets.bottom + 16 }]}>
        ...
        <SpringButton onPress={handleBroadcast} scaleDown={0.96} style={{ flex: 1 }}>
          <View style={[ss.submitBtn, { backgroundColor: theme.colors.primary }]}>
            <Save color="#fff" size={20} strokeWidth={2.5} />
            <Text style={[Typography.headline.sm, { color: '#fff', marginLeft: 8 }]}>Save Changes</Text>
          </View>
        </SpringButton>
      </Animated.View>
