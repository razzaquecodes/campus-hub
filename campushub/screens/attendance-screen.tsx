...
import {
  AlertCircle, ArrowLeft, Calendar, CheckCircle, ChevronRight,
  Clock, TrendingDown, TrendingUp, XCircle, Filter,
...
export function AttendanceScreen() {
  ...
  const counterStyle = useAnimatedStyle(() => ({})); // used for display trigger

  useEffect(() => {
    countVal.value = withDelay(600, withTiming(overallPct, { duration: 1200 }));
  }, []);
          ...
          <View style={{ gap: 10 }}>
            {SUBJECTS.map((sub, i) => {
              ...
              return (
                <Animated.View
                  key={sub.id}
                  entering={FadeInDown.duration(400).delay(i * 60 + 480)}>
